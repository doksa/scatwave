...
function get_multires_size(s,max_res,pad)
   ...
   for res=0,J do      
      local M_p=2^J*torch.ceil((s[s:size()]+pad*2*2^J)/2^J)/2^res
...
function filters_bank.morlet_filters_bank_2D(U0_dim,J,fft,pad)
   local filters={}
   local i=1
   local pad = pad or 1
   
   size_multi_res=get_multires_size(U0_dim,J,pad) -- min padding should be a power of 2
   filters.psi={}   
   filters.size_multi_res=size_multi_res
   
   stride_multi_res=get_multires_stride(U0_dim,J,pad)
   ...
   for j=0,J-1 do
      for theta=1,8 do
         ...
         local psi  = morlet_2d(size_multi_res[1][U0_dim:size()-1], 
                                ...
                                0.8*2^j, 0.5, 3/4*3.1415/2^j, theta*3.1415/8, 0, 1, pad)
...
function gabor_2d(M,N,sigma,slant,xi,theta,offset,fft_shift,pad) 
...
function morlet_2d(M,N,sigma,slant,xi,theta,offset,fft_shift,pad)
   
   local wv=gabor_2d(M,N,sigma,slant,xi,theta,offset,fft_shift,pad)
   local g_modulus=complex.realize(gabor_2d(M,N,sigma,slant,0,theta,offset,fft_shift,pad))
